...
" ===
" === Editor behavior    (neovim 配置)
" ===
set exrc   
set secure
set number         " 显示绝对行号 set nonumber 隐藏
set relativenumber    " 显示相对行号
"set norelativenumber " 隐藏
"set numberwidth=2    "变行号所占用的空间（其中n为空间大小)
""highlight CursorLineNr guifg=#050505    "可以定制当前行号（CursorLineNr）的显示效果



set cursorline      "组高亮显示光标所在的屏幕行
"set cursorcolumn    " 高亮显示光标所在的屏幕列
"set cursorline cursorcolumn        "使用以下命令同时启用光标行和光标列，将显示“十字架”光标
"set nocursorline nocursorcolumn    " 取消高亮显示光标所在的行和列
"highlight CursorLine guibg=lightblue ctermbg=lightgray 



set hidden          "允许隐藏被修改的buffer

"set expandtab      "tab键永远输入的是空格
set noexpandtab     "tab键永远输入的不是空格
set tabstop=2
set shiftwidth=2      "默认用2个空格 缩进宽度2个空格
set softtabstop=2     " 关闭softtabstop 永远不要将空格和tab混合输入
...
set list           "显示非可见字符
"set nolist          " 重新隐藏不可见字符
set listchars=tab:\|\ ,trail:▫



set scrolloff=6    " 保持在光标上下最少行数
set ttimeoutlen=0   " 关闭延时
set notimeout       " 默认超时1000 
...
set indentexpr=     "python缩进


"set foldmethod=manual    " 启用手工折叠使用zm命令，可以手动折叠缩进；而利用zr命令，则可以打开折叠的缩进
set foldmethod=indent    "启用缩进折叠
"set foldmethod=syntax    " 启用语法折叠 help fold-syntax
"set foldmethod=marker    "
"启用标记折叠。所有文本将按照特定标记（默认为{{{和}}}）自动折叠。help fold-marker
"set foldcolumn=N         "命令，将在屏幕左侧显示一个折叠标识列，分别用“-”和“+”而表示打开和关闭的折叠
set foldlevel=99          "可以根据指定的级别折叠缩进
...
set wildmenu       " 在命令行中输入命令时，按下'wildchar'键（默认为Tab）将自动补全命令和参数



set ignorecase     "用于设置大小写敏感 设置为不敏感
"set noignorecase         " 设置为敏感
set smartcase      " 搜索小写字母
"map \s :set smartcase!<CR>:set smartcase?<CR>    "在只需按\ s即可切换。我通常会编写映射，以便还在底行显示该选项是打开还是关闭，如下所示：


set shortmess+=c        " c++ 开发
...
set visualbell       "当输入错误时，屏幕就会闪动然后回到正常状态
"set novisualbell          " 则可以关闭visualbell选项（而使用鸣叫)
"set vb t_vb=              " 如果既不想要鸣叫也不想要屏幕闪烁
...
" Resize splits with arrow keys   (使用箭头键调整拆分大小)
"noremap <up> :res +5<CR>
...
" Press ` to change case (instead of ~) (按 ` 更改大小写（而不是 ~）)
noremap ` ~

noremap <C-c> zz

" Auto change directory to current dir (自动将目录更改为当前目录)
...
" Treesitter   (Neovim 代码高亮插件 nvim-treesitter )
